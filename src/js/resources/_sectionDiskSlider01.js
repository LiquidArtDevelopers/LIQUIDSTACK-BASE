...
const FRAGMENT_SHADER = `
  ...
  void main() {
    ...
    float diskMask = 1.0 - smoothstep(uRadius, uRadius + 0.12, dist);
    ...
    vec2 offset = swirlDir * (ripple + twist) * diskMask;

    vec4 tex0 = texture2D(uTexture0, clamp(uv + offset, 0.0, 1.0));
    vec4 tex1 = texture2D(uTexture1, clamp(uv - offset, 0.0, 1.0));

    float reveal = smoothstep(0.0, 1.0, uProgress + (diskMask * 0.4));
    vec4 color = mix(tex0, tex1, reveal);

    float sheen = diskMask * 0.08 * sin((swirlAngle * 10.0) - (uRotation * 5.0));
    color.rgb += sheen;
...
export default function initSectionDiskSlider01() {
  ...
  sections.forEach((section) => {
    ...
    const uniforms = {
      ...
      uDirection: { value: 1 },
      uRadius: {
        value: clampFloat(section.dataset.diskRadius, 0.18, 0.46, 0.33),
      },
      uStrength: {
        value: clampFloat(section.dataset.diskStrength, 0.2, 1.4, 0.75),
      },
      uRotation: { value: 0 },
    };
