...
function updateProgressUi(progressValue, direction, ringEl, dotEl) {
  if (!ringEl || !dotEl) {
    return;
  }
  ...
  ringEl.style.strokeDashoffset = `${circumference * (1 - clamped)}`;

  const signed = direction >= 0 ? clamped : 1 - clamped;
  const angle = (-0.5 + signed) * (Math.PI * 2);
  const cx = 50 + Math.cos(angle) * radius;
  const cy = 50 + Math.sin(angle) * radius;
  dotEl.setAttribute('cx', cx.toFixed(3));
  dotEl.setAttribute('cy', cy.toFixed(3));
}
...
export default function initSectionDiskSlider01() {
  ...
  sections.forEach((section) => {
    ...
    const progressDot = section.querySelector('[data-disk-progress-dot]');
    ...
    const syncProgressRadius = () => {
      if (!progressRing || !progressDot || !overlayDisk) {
        return;
      }
    ...
    const renderLoop = () => {
      ...
      updateProgressUi(uniforms.uProgressRing.value, uniforms.uDirection.value, progressRing, progressDot);
